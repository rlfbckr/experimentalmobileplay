...
function setup() {
  ...
  setInterval(updateThingsToServer, 5000); 
...
function updateThingsToServer() {
  for (let i = 0; i < things.length; i++) {
    var position = myMap.pixelToLatlng(things[i].x, things[i].y);
    firebase.database().ref('things/' + things[i].id).set({
...
function gen_uid() {
  /*
   erzeuge eine user id anbhänig von bildschirmaufläsung; browser id, etc....
...
class Thing {
  ...
  move() {
    ...
    this.diameter = map(this.age(), 0, this.maxage, 100, 5);
